...
with c12:
    st.info(
        ...
        "Os autores aplicaram **SMOTE** para gerar ~1.600 amostras adicionais, "
        "totalizando 2.111 registros.\n\n"
        "Isso contribui para as acurácias altas do modelo (~83–85% sem leakage). "
        "Na sidebar você pode explorar apenas os dados originais."
    )
...
with col_i1:
    ...
    st.success(
        "**🥦 Consumo de Vegetais**\n\n"
        "Quanto mais grave o nível de obesidade, menor o consumo de vegetais. "
        "Orientação nutricional deve ser prioridade no acompanhamento."
    )
    st.info(
        "**👨‍👩‍👧 Histórico Familiar × Alimentação**\n\n"
        "A combinação de histórico familiar positivo com consumo frequente de "
        "alimentos calóricos é especialmente prevalente nos níveis mais graves. "
        "Esses pacientes merecem acompanhamento preventivo redobrado."
    )
...
with col_i2:
    ...
    st.warning(
        ...
        "O desequilíbrio entre quantidade de refeições e atividade física "
        "cresce progressivamente com a gravidade da obesidade."
    )
    st.error(
        "**🍺 Álcool**\n\n"
        "O consumo frequente de álcool está associado aos níveis mais graves. "
        "Triagem de consumo alcoólico deve integrar a avaliação de risco metabólico."
    )
...
with st.expander("📄 Ver tabela de dados filtrados"):
    ...
    st.dataframe(dff[cols_exibir].rename(columns=rename),
                 use_container_width=True, hide_index=True)

st.caption("📊 Dashboard analítico — POSTECH Tech Challenge Fase 04 — Data Analytics")
